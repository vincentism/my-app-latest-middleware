...
async function handleSubscriptionUpdate(subscription, env) {
  ...
  await env.MY_KV.put(`subscription_by_id:${subscription.id}`, JSON.stringify(subscriptionData));
...
async function handlePaymentSucceeded(invoice, env) {
  ...
  await env.MY_KV.put(`payment:${invoice.id}`, JSON.stringify(paymentData));
