...
export async function authenticateRequest(request, env, options = {}) {
  try {
    ...
    if (!JWT_SECRET) {
      console.error('❌ JWT_SECRET 未配置');
      return { success: false, error: "JWT_SECRET environment variable is not set.", status: 500 };
    }
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      ...
      return { success: false, error: 'Authorization header is missing or invalid', status: 401 };
    }
    ...
    console.error("💥 Edge Function认证错误:", error.message);
    console.error("错误堆栈:", error.stack);
    ...
    return { success: false, error: `Authentication error: ${error.message}`, status: 500 };
  }
}
